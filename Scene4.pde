...
class scene4 {
  ...
  float ballX; // Position of the red ball
  float ballSpeed; // Speed of the red ball
  
    scene4(PApplet parent) {
    music3 = new SoundFile(parent, "asset/scene4.mp3");
  }
  ...
  void setup() {
    ...
    ballX = 507.5; // Initial position of the red ball
    ballSpeed = 4.5; // Speed of the red ball
    music3.play();
  }

  void draw() {
    int duration = 720; // Duration in frames
    if (frameCount - startFrame >= duration) {
      isFinished = true;
    }

    // Flip the fish every 60 frames
    if (frameCount % 60 == 0) {
      flipFish = !flipFish;
    }
    ...
    // Draw the red ball if it's within the screen
    if (ballX < width + 35.5) {
      fill(233, 46, 46);
      stroke(0);
      strokeWeight(3);
      ellipse(ballX, 446.5, 71, 65);
      fill(236, 93, 93);
      stroke(0);
      strokeWeight(3);
      ellipse(ballX, 445.5, 71, 27);
    }
  }
